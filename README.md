...
For `qa process`, `--rows-file` accepts either raw process rows as JSON/JSONL or the full JSON report emitted by `tiangong-lca process list --json`, as long as it contains a `rows` array. The deterministic process gate blocks an Input exchange that references the exact UUID and version of the quantitative-reference Output flow. A legitimate closed-loop recirculation is exempted only by a scoped `tiangongfoundry:stageFlowSelfProviderExemption` object under process `dataSetInformation/common:other`; it must bind the input exchange and reference-flow identity and include source evidence, modeling justification, and a durable review-decision id.
