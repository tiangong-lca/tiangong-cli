...
`apply` is write-disabled unless all three commit guards are present: `--commit`, `--approve-plan <sha256>`, and `--confirm <current-account-email>`. Before the first write it re-checks the whole plan for drift and persists `approval-record.json`. Ordinary draft updates/deletes use their platform paths. Each alias dimension is sent once to `cmd_dataset_alias_batch_guarded` with `target_visibility=owner_draft`; the CLI never falls back to 52 sequential writes. The RPC row-locks the complete submitted closure, checks actor ownership, state 0, exact payload/timestamp locks and embedded UUID/version, rejects missing or phantom flow/exchange references, and returns one summary audit id plus one audit id per row. The CLI writes per-row `apply-progress.jsonl`, per-exchange `alias-exchange-progress.jsonl`, and batch-last `alias-batch-progress.jsonl`, all bound to owner-draft mode. A lost response or incomplete derived ledger is repaired only by replaying the same whole batch and matching every returned proof.
