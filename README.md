...
Review note, 2026-07-16: `dataset maintenance flow-identity capture|plan|freeze|seal-approval|run|verify` is the dedicated Step 3 workflow for approved BAFU elementary-flow identity mappings. Capture performs one complete authenticated census, stores only the reviewed process reference closure, and makes exactly one database-attestation POST; it does not perform scope writes or client-side derivative-baseline RPCs. Plan permanently rejects pre-Step2/224-row authorities and binds the exact 305-source semantic request to the immutable database receipt. Execution uses thin ordinal/intent-proof requests against the receipt-owned rewrite ledger in strict order. The runner reads scope at invocation start, after any ambiguous response, after the primary batch, and while final derivatives are pending; it submits no finalize while derivatives are pending and, only after the exact causal/current closure is ready, submits at most one finalize POST in that invocation. A local attempt marker never permits automatic process or finalize replay. Terminal verification independently proves unchanged source/public/support rows, exact desired owner-draft processes, zero approved-source reference residue, unchanged pending/blocker/orphan closure, and causal derivative completion. Failed/stale derivatives stop the runner; derivative-only compensation requires a separate future plan, freeze, and exact approval and must never replay the process mutation.
