...
No-Node bootstrap scripts and their separately pinned adjacent lock, final component publication, all-platform launch qualification and C1/F1 release integration remain required before #274/W07 is complete. The private live proof installs actual Node 24.19 and public CLI 0.1.9 production files from a verified local seed, invokes real account identity through that managed pair, checks offline reuse and lease/prune behavior, and preserves original inputs. It does not claim that a public runtime component has already been released.
