...
lastReviewedNote: 'Reviewed for Issue #232: records identity-bound resource-aware scheduling, internal lock ownership, timer limits, dataset dogfood, and closed package boundaries at version 0.1.1.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
- `@tiangong-lca/cli/batch` separates overall run identity from exact per-item identity/content/policy contracts. It preflights every projection before unsafe work and rechecks identity/content/policy/resource before resumed acceptance or claim. Identity drift yields `BatchItemIdentityDriftError` plus `item_identity_drift` without execution. It caps concurrency at 64 and serializes matching exclusive keys by leaving blocked items unclaimed while scanning later free keys, so waiting does not consume worker capacity or outrun stop. It exposes input, resource-aware claim, and completion order and awaits a monotonic event sink. Read retry is explicitly classified and timer-capped; mutation retry is configuration-invalid, and consumed attempts can proceed only through explicit readback recovery.
