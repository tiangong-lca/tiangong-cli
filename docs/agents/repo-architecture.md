...
Review note, 2026-08-02: Issue #216 v3 closure separates the immutable consumer `sourceTreeCommit` from the delivery HEAD. `scripts/scan-data-api-consumers.ts` parses the governed Git blobs with the TypeScript compiler AST and compares its independently derived, globally unique occurrence set bidirectionally with `contracts/supabase-consumer-manifest.v3.json`. The filtered tree digest excludes exactly that audit tool and rejects every other governed-source drift. The canonical Schema fixes each occurrence's exact span, operation, transport, credential, schema/object, signature, ACL, semantics, and upstream links. The artifact remains permanently candidate-only: database-engine#357 must verify its exact bytes, and Edge/Release/hosted lifecycle evidence plus workspace#484 integration gates remain external blockers.
