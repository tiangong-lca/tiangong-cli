...
lastReviewedNote: 'Reviewed for Issue #230: 0.1.1 release proof now test-locks the reusable four-platform pre-tag gate and one credential-free provenance, tarball-integrity, and public pnpm consumer verifier.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
Review note, 2026-08-25: Issue #230 release proof requires pre-mutation 0.1.1 npm/tag absence, exactly four live CLI-version fixtures at 0.1.1, unchanged `pnpm-lock.yaml`, strict Docpact, package/prepush/exact-coverage gates, and the four-platform matrix on both the PR head and exact release merge. The merge-triggered tag job may run only after the reusable macOS arm64, Ubuntu x64/arm64, and Windows x64 matrix passes. After native pnpm Trusted Publishing, `release:verify-published` must bind optional registry `gitHead`, mandatory SLSA provenance `gitCommit`, tag/workflow/run identity, independently downloaded tarball sha512, and a credential-free public pnpm bin/ESM/CJS consumer including `auth identity-receipt --help` before workspace integration.
...
- `.github/workflows/quality-gate.yml` supports manual exact-head reproduction and reusable invocation; a detected CLI version change in `tag-release-from-merge.yml` must pass all four jobs before tag creation
...
- `pnpm release:verify-published -- --version <x.y.z> --expected-git-head <release-merge-sha>` is the post-publish registry/provenance/tarball/public-consumer gate; it passes no TianGong or registry credential into the clean consumer
