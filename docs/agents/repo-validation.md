...
lastReviewedNote: 'Reviewed for Issue #232: proof covers infrastructure fatal/drain behavior, guarded identity drift, FIFO/min-heap scale, locks, timers, packed consumers, dogfood, and 100% coverage.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| `src/command-spec.ts`, `src/batch.ts`, declaration config, or public package exports | focused public primitive tests; `pnpm typecheck`; `pnpm test:package`; `pnpm test:coverage`; `pnpm test:coverage:assert-full` | `pnpm prepush:gate`; run cross-process run-lock, infrastructure-drain, scale-operation, and packed ESM/CJS/TS proofs | Preserve exact CommandSpec authority and adapter cleanup, Node timer limits, guarded claim-time identity/content/policy/resource binding, immediate fatal claim closure plus settled worker drainage, mutation no-auto-retry, per-resource FIFO/min-ready-heap scheduling and event/claim order, successor exposure only after stop, per-result resume stop, one run-directory lock domain with live-scope drainage, internal-only ownership metadata, host-aware stale handling, and a closed package root. |
