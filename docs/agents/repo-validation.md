...
lastReviewedNote: 'Reviewed for Issue #232: proof covers exact CommandSpec authority, projection/resume/resource scheduling, detached-scope locks, packed ESM/CJS/TS consumers, dogfood parity, and 100% coverage.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
Review note, 2026-08-26: Issue #232 proof covers exact Foundry CommandSpec differential vectors; pre-aborted zero-spawn, synchronous adapter failure cleanup, and timeout/abort race containment; all-item projection preflight; per-item content/policy resume triples; runtime exclusive-key validation and serialization; per-result resume stop; 64-worker resource ceiling; mutation retry rejection and readback-only recovery; monotonic awaited events; live-owner-scope-only reentrancy; detached nested scope drainage; completed-context rejection; same-directory cross-process exclusion across identities; same-host stale recovery; foreign/live lock preservation; and physical-lock-safe local handoff. Package proof runs the explicit launcher, CommandSpec, batch, and run-lock APIs from packed ESM/CJS hosts and compiles generated declarations with empty type roots. Dataset save-draft proof locks serial-prefix behavior, suffix concurrency/fatal stop, row order, and exact report bytes. Version stays 0.1.1 until separate release work.
...
| `src/command-spec.ts`, `src/batch.ts`, declaration config, or public package exports | focused public primitive tests; `pnpm typecheck`; `pnpm test:package`; `pnpm test:coverage`; `pnpm test:coverage:assert-full` | `pnpm prepush:gate`; run cross-process run-lock and packed ESM/CJS/TS proofs | Preserve exact CommandSpec authority and adapter cleanup, per-item projection/resume hashes, mutation no-auto-retry, runtime-string exclusive-key event order, per-result resume stop, one run-directory lock domain with live-scope drainage, host-aware stale handling, and a closed package root. |
