...
Review note, 2026-08-02: Issue #216 v3 proof runs `npm run lint:data-api-consumers` against immutable Git blobs and the checked-in canonical manifest bytes. It must reject unresolved dynamic/bracket targets, duplicate or missing occurrences, exact-span tamper, non-canonical bytes, non-regular governed files, canonical-origin drift, source/delivery SHA or filtered-tree drift, and any authority other than candidate/false/false. The pull-request workflow runs focused positive/negative tests and typecheck. This proof reports public residue, pending capabilities, and explicit absence evidence, but never authorizes database freeze or hosted mutation; database-engine#357 exact-byte verification and the Edge/Release/hosted joint lifecycle remain required.
