...
- Alias-batch tests must prove exact `time`/`length_time` and `target_mode=owner_draft` binding; factors `0.00011415525114155251`/`1000` without binary floating point; 52 row and 59 exchange identities; embedded UUID/version preservation; 309 unrelated exchanges unchanged; source-reference zeroing and fixed target postconditions; exactly one `target_visibility=owner_draft` `cmd_dataset_alias_batch_guarded` call per dimension; lost-response replay; rejection of public/foreign/mixed/stale rows; and rejection of numeric, foreign, duplicate, or mismatched audit ids. CLI accepts audit ids only as positive-integer strings so PostgreSQL bigint values cannot lose precision in JavaScript. Verify validates returned audit ids and their local proof correlation, but does not independently query `public.command_audit_log`; the guarded RPC remains responsible for audit-row creation and replay proof.
