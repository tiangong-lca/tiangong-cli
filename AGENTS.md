...
lastReviewedNote: 'Reviewed for Issue #244: CLI authentication now includes Supabase OAuth 2.1 Authorization Code with S256 PKCE, private atomic refresh-token sessions, local-only status, live redacted whoami/doctor-auth, explicit headless access tokens, and bounded legacy bootstrap compatibility.'
related:
  - .docpact/config.yaml
