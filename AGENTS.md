...
lastReviewedNote: 'Reviewed for Issue #230: 0.1.1 preserves runtime/auth dependencies while requiring exact-platform Node 24 gates, cryptographic Sigstore and registry proof, isolated pnpm consumers, and finish-first tracked integration.'
related:
  - .docpact/config.yaml
...
- Release tagging is guarded in `.github/workflows/tag-release-from-merge.yml` so only the upstream repository can execute the merge-tag flow. Its detector runs under pinned Node 24; it calls the reusable four-platform `.github/workflows/quality-gate.yml` only for a CLI release, every job asserts exact runtime platform/architecture, and `cli-v<version>` tag creation depends on all four. `.github/workflows/publish.yml` publishes from that tag and also supports `workflow_dispatch` for existing-tag recovery/backfill.
