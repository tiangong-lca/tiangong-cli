...
Review note, 2026-08-25: Issue #228 implements `auth identity-receipt` as the CLI-owned live identity proof. It resolves the existing API-key/session chain, validates the canonical Supabase project before auth work, performs a bounded read-only `/auth/v1/user` lookup, retries only one 401/403 through forced session refresh, and emits an exact-key receipt containing no credential, token, full email, session path, or credential-derived fingerprint. A production guard is valid only with both expected project and user argv assertions and `assertions.mode=intent-bound`. The companion TypeScript case runner reads only three allowlisted values from the ignored Foundry env file, disables cache, uses a clean cwd and argv-array spawn, and never persists raw child output.
