...
lastReviewedNote: 'Reviewed for Issue #230: the 0.1.1 release preserves runtime/auth/dependencies while making the four-platform pnpm gate a pre-tag dependency and adding executable public provenance/consumer and tracked-integration proof.'
related:
  - .docpact/config.yaml
...
Review note, 2026-08-25: Issue #230 publishes the merged Issue #228 runtime as CLI 0.1.1 without changing command runtime, dependencies, auth behavior, credentials, or dataset operations. Independent review hardened the release control plane: a detected CLI version change calls the reusable four-platform pnpm gate and tag creation depends on its success; `release:verify-published` binds optional registry `gitHead`, mandatory SLSA provenance `gitCommit`, tag/workflow/run identity, actual tarball sha512, and credential-free public pnpm ESM/CJS/bin consumers; workspace integration returns to the current tracked controller.
...
- Release tagging is guarded in `.github/workflows/tag-release-from-merge.yml` so only the upstream repository can execute the merge-tag flow. It detects version changes first, calls the reusable four-platform `.github/workflows/quality-gate.yml` only for a CLI release, and makes `cli-v<version>` tag creation depend on all four jobs. `.github/workflows/publish.yml` publishes from that tag and also supports `workflow_dispatch` for existing-tag recovery/backfill.
