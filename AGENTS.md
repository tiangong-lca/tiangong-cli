...
lastReviewedNote: 'Reviewed for Issue #232: identity drift, resource-aware claims, internal-only lock ownership, Node timer bounds, dataset dogfood, and packed declarations are documented while version remains 0.1.1.'
related:
  - .docpact/config.yaml
...
- Public batch safety: all item identity/content/policy/resource projections validate before work and again before resumed acceptance or fresh claim; identity drift has an explicit error/event and zero execution. Exclusive keys must be runtime strings, same keys serialize, blocked keys remain unclaimed without occupying workers, and later free keys retain bounded concurrency. Mutation retry is rejected, incomplete attempts require explicit readback recovery, resume requires exact run/item contracts, every pre-claim result can trigger stop, and event delivery is monotonic plus awaited. Retry policy/backoff delays cannot exceed Node's timer maximum.
