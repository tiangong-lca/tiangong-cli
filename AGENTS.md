...
lastReviewedNote: 'Reviewed for Issue #247: OAuth/session state-lock metadata uses one read attempt, treats only concurrent ENOENT as a released lock, and continues to fail closed on every other read error.'
related:
  - .docpact/config.yaml
