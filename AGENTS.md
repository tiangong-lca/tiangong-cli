...
lastReviewedNote: 'Reviewed for Issue #230: the 0.1.1 release-only change preserves the auth, pnpm/TS7, package, provenance, and integration contracts.'
related:
  - .docpact/config.yaml
...
Review note, 2026-08-25: Issue #230 publishes the merged Issue #228 runtime as CLI 0.1.1 through a release-only metadata/fixture change. It introduces no command, dependency, auth behavior, credential, dataset operation, tag rule, or alternate publish path. Exact four-platform gates, merge-triggered tag creation, native pnpm Trusted Publishing/provenance, public consumer proof, and workspace integration remain mandatory.
