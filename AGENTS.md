...
lastReviewedCommit: 9a94fee5d40829d124c0311e5a32654b25f0a090
lastReviewedNote: 'Reviewed for Issue #216: all Data API calls are governed by one exact-contract manifest and explicit public/api profiles; unresolved private-target capabilities fail closed.'
related:
  - .docpact/config.yaml
...
Review note, 2026-08-01: Issue #216 freezes the immutable pre-Contract CLI Data API inventory regenerated by database-engine#353 at artifact merge `94bfefe159c949da1b1cc1d25718961050baaa1a`, catalog replay input `databaseSchemaSha=20f56228c21e8e677154c3e77fbf0e243dde677d`, and artifact SHA-256 `d7353b0b3d2dcd3bcc64ffaf41ff2015729142789e0b3a39818acc12ebf35c16`. The versioned `legacy-public-v1` and `api-contract-v1` profiles always send an explicit schema. Nine core relations remain explicitly `public`; RPCs use the manifest-selected profile. `cmd_dataset_alias_plan_guarded` fails closed in `api-contract-v1` until database-engine#358 publishes an authenticated `api` replacement; the CLI does not expose `anon` or `service_role` credentials. GET/HEAD and only manifest-classified read RPCs may refresh authentication and replay once after 401/403; relation writes, mutation RPCs, and unknown RPCs are never automatically replayed.
...
- Issue #216 consumer closure is represented by `contracts/supabase-consumer-manifest.v3.json` and its canonical JSON Schema. The artifact is always candidate-only and non-authorizing. `scripts/scan-data-api-consumers.ts` independently derives the exact occurrence set from immutable Git blobs with the TypeScript compiler AST, while `.github/workflows/supabase-consumer-closure.yml` enforces the same proof on pull requests. It cannot authorize database freeze, hosted mutation, merge, deployment, or release.
